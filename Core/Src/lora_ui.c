...
void Menu_ui(void)
{
	...
	i=20;
	z=30;
	Show_Str(40+z,50+i,200,16,"KEY0:下一个",16,0); 				    	 
	Show_Str(40+z,70+i,200,16,"KEY1:上一个",16,0);				    	 
	Show_Str(40+z,90+i,200,16,"WK_UP:确定/取消",16,0); 
	POINT_COLOR=RED;
	x=30;
	y=95;
	Show_Str(40+x,45+y,200,16,"进入通信",16,0);
	Show_Str(40+x,70+y,200,16,"模块地址：",16,0);
	Show_Str(40+x,95+y,200,16,"通信信道：",16,0);
	Show_Str(40+x,120+y,200,16,"空中速率：",16,0);
	Show_Str(40+x,145+y,200,16,"发射功率：",16,0);
	Show_Str(40+x,170+y,200,16,"工作模式：",16,0);
	Show_Str(40+x,195+y,200,16,"发送状态：",16,0);
	
	POINT_COLOR=BLUE;
	Show_Str(x+10,45+y,200,16,"→",16,0); 
	
	x=110;
	y=95;
	num = num_count(LoRa_CFG.addr);
	LCD_ShowNum(40+x+10,70+y,LoRa_CFG.addr,num,16);
	num = num_count(LoRa_CFG.chn);
	LCD_ShowxNum(40+x+10,95+y,LoRa_CFG.chn,num,16,0);
	Show_Str(40+x+10,120+y,200,16,(u8*)cnttbl_rate[LoRa_CFG.wlrate],16,0);
	Show_Str(40+x+10,145+y,200,16,(u8*)cnttbl_power[LoRa_CFG.power],16,0);
	Show_Str(40+x+10,170+y,200,16,(u8*)cnttbl_mode[LoRa_CFG.mode],16,0);
	Show_Str(40+x+10,195+y,200,16,(u8*)cnttbl_sta[LoRa_CFG.mode_sta],16,0);
	  ...
	  if(key==KEY1_PRES||key==KEY0_PRES)
	  {
		  switch(num)
		  {
			  case 1: 
					...
					Show_Str(40+x+10,70+y,200,16,"                ",16,0);
					LCD_ShowNum(40+x+10,70+y,LoRa_CFG.addr,count,16);
					break;
			 case 2: 
					count = num_count(LoRa_CFG.chn);
			        Show_Str(40+x+10,95+y,200,16,"                ",16,0);
			        LCD_ShowxNum(40+x+10,95+y,LoRa_CFG.chn,count,16,0);
					break;
			 case 3:
				    Show_Str(40+x+10,120+y,200,16,"             ",16,0);
				    Show_Str(40+x+10,120+y,200,16,(u8*)cnttbl_rate[LoRa_CFG.wlrate],16,0);
					break;
			 case 4:
				    Show_Str(40+x+10,145+y,200,16,(u8*)cnttbl_power[LoRa_CFG.power],16,0);
...
u32 obj_addr=0;//记录目标地址和信道(用户通过键盘数据输入时最大出现99999,所以需要定义u32类型）
u8 obj_chn=0;
